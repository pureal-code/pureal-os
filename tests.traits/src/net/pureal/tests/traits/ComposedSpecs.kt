...
class ComposedSpecs : Spek() {{
    given("a composed element of a rectangle and a circle") {
        val c = coloredElement(content = Unit.VALUE, shape = circle(0.6), fill = Fills.solid(Colors.red))
        val r = coloredElement(content = Unit.VALUE, shape = rectangle(vector(1, 1)), fill = Fills.solid(Colors.blue))
