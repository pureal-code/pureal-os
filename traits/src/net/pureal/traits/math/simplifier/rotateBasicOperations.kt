...
fun rotateBasicOperations(it: Real): Real {
    ...
    fun rotateInnerAdd(it: Real): Real {
        ...
        fun pn_add(it: Real): pnRealArray {
            if(it !is AdditionValue && it !is SubtractionValue) return pnRealArray(array(rotateBasicOperations(it)), array())
        ...
        val pn = pn_add(it)
        val p = (pn.p.filter { it.isPositive } + pn.n.filter { !it.isPositive && !it.isZero }.map { -it }).copyToArray()
        val n = (pn.n.filter { it.isPositive } + pn.p.filter { !it.isPositive && !it.isZero }.map { -it }).copyToArray()
